...
public class ProducerController {
	...
	@Autowired
	private MsgSender senderMessages;
	...
	public String topicSender() throws AmqpException, UnsupportedEncodingException {
		MsgTxtBo msgTxtBo = new MsgTxtBo();
		msgTxtBo.setMsgId(UUID.randomUUID().toString().replace("-",""));
		msgTxtBo.setProductNo(11);
		msgTxtBo.setOrderNo(12);
//		if(null == routingKey) {
//			routingKey="changsha.kf";
//		}
//		MessageProperties messageProperties = new MessageProperties();
//		messageProperties.setContentType(MessageProperties.CONTENT_TYPE_TEXT_PLAIN);
		//fanout模式只往exchange里发送消息。分发到exchange下的所有queue
//		rabbitTemplate.send("topicExchange", routingKey, new Message(message.getBytes("UTF-8"),messageProperties));
		senderMessages.senderMsg(msgTxtBo);
		return "message sended : routingKey >";
	}
